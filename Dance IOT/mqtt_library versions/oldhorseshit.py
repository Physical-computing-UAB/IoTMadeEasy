...
#    httpwww.eclipse.orglegalepl-v10.html
# and the Eclipse Distribution License is available at
#   httpwww.eclipse.orgorgdocumentsedl-v10.php.
#
# Contributors
#    Roger Light - initial API and implementation


This is an MQTT v3.1 client module. MQTT is a lightweight pubsub messaging
...
# To initialize start variable without knowing actual type 
start = time.time()
end = time.time()
start = end- start
...
def topic_matches_sub(sub, topic)
    ...
    foobar would match the subscription foo# or +bar
    nonmatching would not match the subscription non++
...
def _socketpair_compat()
    TCPIP socketpair including Windows support
...
class MQTTMessageInfo(object)
    ...
    message has been published, andor wait until it is published.
...
class MQTTMessage(object)
    ...
    topic  Stringbytes. topic that the message was published on.
    payload  Stringbytes the message payload.
...
class Client(object)
    MQTT version 3.13.1.1 client class.
    ...
    def tls_set_context(self, context=None)
        Configure network encryption and authentication context. Enables SSLTLS support.
        ...
        if self._ssl_context is not None
            raise ValueError('SSLTLS has already been configured.')
        ...
        if context is None
            if hasattr(ssl, 'create_default_context')
                ...
                raise ValueError('SSLTLS context must be specified')
    ...
    def tls_set(self, ca_certs=None, certfile=None, keyfile=None, cert_reqs=None, tls_version=None, ciphers=None)
        Configure network encryption and authentication options. Enables SSLTLS support.
        ...
        tls_version allows the version of the SSLTLS protocol used to be
        ...
        Must be called before connect() or connect_async().
        if ssl is None
            raise ValueError('This platform has no SSLTLS.')
    ...
    def connect(self, host, port=1883, keepalive=60, bind_address=)
        ...
        1883. Note that the default port for MQTT over SSLTLS is 8883 so if you
    ...
    def connect_srv(self, domain=None, keepalive=60, bind_address=)
        ...
        try
            ...
            raise ValueError(No answerNXDOMAIN for SRV in %s % (domain))
    ...
    def loop(self, timeout=1.0, max_packets=1)
        ...
        timeout The time in seconds to wait for incomingoutgoing network
        ...
        # Use global start and end variables     
        global start
        global end
        #To represent local start time
        ...
        try
            ...
            # Can occur if we just reconnected but rlistwlist contain a -1 for
            # some reason.
            return MQTT_ERR_CONN_LOST
        ...
        if self._sock in socklist[0] or pending_bytes  0
            ...
            if rc or self._sock is None
                return rc
        print(self.rakR)        
        # temp={'R' self.rakR, 'L' self.rakL, 'H' self.rakH}
        # print(temp)
        # Json serialization using json.dump 
        time.sleep(1)
        out_file= open(r'CUsersRakshitDownloadspat3.json', 'w')
        json.dump(temp, out_file)
        
        # Compute end time and subtract local_start from end time to compute local elapsed time and add it to start to compute overall elapsed time
        end = time.time()
        start = (end - local_start)+start

        # Print function remove after computing efficiency
    ...
    def publish(self, topic, payload=None, qos=0, retain=False)
        ...
        you wish to send a true intfloat, use struct.pack() to create the
        payload you require.
        qos The quality of service level to use.
        retain If set to true, the message will be set as the last known
        goodretained message for the topic.
    ...
    def username_pw_set(self, username, password=None)
        ...
        username The username to authenticate with. Need have no relationship to the client id. Must be unicode    
            [MQTT-3.1.3-11].
            Set to None to reset client back to not using usernamepassword for broker authentication.
    ...
    def subscribe(self, topic, qos=0)
        ...
        ------------------------
        e.g. subscribe([(mytopic, 0), (anothertopic, 2)])
    ...
    def max_inflight_messages_set(self, inflight)
        Set the maximum number of messages with QoS0 that can be part way
    ...
    def message_retry_set(self, retry)
        Set the timeout in seconds before a message with QoS0 is retried.
    ...
    def will_set(self, topic, payload=None, qos=0, retain=False)
        ...
        that number. If you wish to send a true intfloat, use struct.pack() to
    ...
    def _packet_read(self)
        ...
        if self._in_packet['have_remaining'] == 0
            ...
            # httppublib.boulder.ibm.cominfocenterwmbhelpv6r0m0topiccom.ibm.etools.mft.docac10870_.htm
            while True
                try
                    ...
                    # Anything more likely means a brokenmalicious client.
    ...
    def _send_connect(self, keepalive, clean_session)
        ...
        protocol = bMQTT if proto_ver = MQTTv311 else bMQIsdp  # hard-coded UTF-8 encoded string
...
class WebsocketWrapper(object)
    ...
    def _do_handshake(self, extra_headers)
        ...
        websocket_headers = {
            Host {self._hosts}{self._portd}.format(self=self),
            Upgrade websocket,
            Connection Upgrade,
            Origin https{self._hosts}{self._portd}.format(self=self),
        ...
        header = rn.join([
            GET {self._path} HTTP1.1.format(self=self),
            rn.join({} {}.format(i, j) for i, j in websocket_headers.items()),
            rn,
        ]).encode(utf8)
        ...
        while True
            ...
            if byte == bn
                if len(self._readbuffer)  2
                    # check upgrade
                    if bconnection in str(self._readbuffer).lower().encode('utf-8')
                        if bupgrade not in str(self._readbuffer).lower().encode('utf-8')
                            raise WebsocketConnectionError(WebSocket handshake error, connection not upgraded)
                        else
                            has_upgrade = True

                    # check key hash
                    if bsec-websocket-accept in str(self._readbuffer).lower().encode('utf-8')
    ...
    def _buffered_read(self, length)
        ...
        return self._readbuffer[self._readbuffer_head - lengthself._readbuffer_head]
    ...
    def _recv_impl(self, length)
        ...
        try
            ...
            if readindex  0
                ...
                result = payload[chunk_startindexreadindex]
